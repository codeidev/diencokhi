...
export default function AddCategory() {
  ...
  useEffect(() => {
    const getCategories = async () => {
      ...
      const res2 = await fetch('http://localhost:3000/categories3');
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch('http://localhost:3000/addcategory3', {
      method: 'POST',
      body: data,
    });
  ...
  return (
    <div className="m-3 title-fix">
      ...
      <form onSubmit={handleSubmit} encType="multipart/form-data">
        ...
        <button type="submit" className="btn btn-primary my-3">Thêm đồ án - luận văn</button>
      </form>
    </div>
  );
}